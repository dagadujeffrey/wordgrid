...
test('hard AI prefers completing the highest scoring word', async () => {
  ...
  applyMove(board, { row: 0, col: 0, letter: 'C', playerId: 'ai' });
  applyMove(board, { row: 0, col: 1, letter: 'A', playerId: 'ai' });

  const move = await withSeededRandom(0, () => chooseMove(board, 'ai', { difficulty: 'hard' }));
  ...
  applyMove(clone, { row: 0, col: 0, letter: 'C', playerId: 'ai' });
  applyMove(clone, { row: 0, col: 1, letter: 'A', playerId: 'ai' });
